...
def write_last_log_to_csv(source_log, output):
   result_dict = {}
   data = []
   with open(source_log) as f, open(output, 'w') as w:
       reader = csv.reader(f)
       writer = csv.writer(w, quoting=csv.QUOTE_NONNUMERIC)
       headers = next(reader)
       writer.writerow(headers)
       for nick, email, date in reader:
           data.append([nick, email, convert_str_to_datetime(date)])
       data = sorted(data, key=operator.itemgetter(2))
       for line in data:
           result_dict.update({ line[1] : line[2] })
       for key, value in result_dict.items():           
           for nick, email, date in data:             
               if key == email and value == date:
                   writer.writerow([nick, email, convert_datetime_to_str(date)])
 
if __name__ == "__main__": 
      write_last_log_to_csv("mail_log.csv", "mail_log_out.csv") 
